...
describe("Benture Dividend Distributing Contract", () => {
    ...
    describe("Unlock tokens", () => {
        ...
        it("Should lock and unlock multiple clients` tokens before any distribution", async () => {
            ...
            await origToken
                .connect(clientAcc2)
                .approve(benture.address, mintAmount.mul(2));
            ...
            await benture
                .connect(clientAcc2)
                .lockTokens(origToken.address, mintAmount);
            ...
            await benture
                .connect(clientAcc1)
                .unlockTokens(origToken.address, mintAmount.div(2));
            await benture
                .connect(clientAcc2)
                .unlockTokens(origToken.address, mintAmount.div(2));
    ...
    describe("Fails for Custom Dividends", () => {
        ...
        it("Should fail to distribute custom dividends if any list is empty", async () => {
            let { benture, factory, adminToken, origToken, distToken } =
                await loadFixture(deploys);
            await expect(
                benture.distributeDividendsCustom(distToken.address, [], [1, 2])
            ).to.be.revertedWithCustomError(benture, "EmptyList");
