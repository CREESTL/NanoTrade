...
describe("Benture Admin Token", () => {
    ...
    describe("Getters", () => {
        ...
        it("Should verify that user controls provided ERC20 token", async () => {
            ...
            expect(
                await adminToken.checkAdminOfProject(
                    ownerAcc.address,
                    token.address
                )
            ).to.equal(true);
        ...
        it("Should fail to verify that zero address controls provided ERC20 token", async () => {
            ...
            await expect(
                adminToken.checkAdminOfProject(zeroAddress, token.address)
            ).to.be.revertedWithCustomError(adminToken, "InvalidUserAddress");
        });
        ...
        it("Should fail to verify that user controls provided token with zero address", async () => {
            ...
            await expect(
                adminToken.checkAdminOfProject(clientAcc1.address, zeroAddress)
        ...
        it("Should check that user is admin of some project", async () => {
            let {
                ownerAcc,
                clientAcc1,
                clientAcc2,
            ...
            expect(await adminToken.checkAdminOfAny(ownerAcc.address)).to.equal(
                true
            );
            expect(
                await adminToken.checkAdminOfAny(clientAcc1.address)
            ).to.equal(false);
        });
    });
